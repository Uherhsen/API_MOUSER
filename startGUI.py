...
from GetDataMouser_with_openpyxl import GetData as GD 
from tkinter import *
...
def visibilityArt():
    if lbox.winfo_ismapped():
        ...
        Art_visibility_button['bg']='SlateGray2'
        ...
        Art_visibility_button['bg']='SlateGray1'
        f1.pack(side=LEFT)
...
def TMEsearch_menu_command():
    ...
    webbrowser.open_new('https://ru.mouser.com/Search/Refine?Keyword='+searchname)#+'&s_field=1000011&s_order=desc') https://ru.mouser.com/ProductDetail/
...
def editor_menu_command():
    ...
    def inputreplase(event):
        '''Функция с полями для ввода/редактирования замен, включается по двойному щелчку мышью'''
        # кнопка >>>
        def confirm():
...
def about_application():
    inform = " Это программа для получения информации по артикулам товаров с сайта MOUSER (Mouser Electronics  - https://ru.mouser.com), а также по составлению краткого описания в контексте ТН ВЭД с использованием полученной информации.\n Для получения информации используются команды программного интерфейса(API) сайта MOUSER и парсинг. Описания составляются на основе базы текстовых шаблонов составляемых пользователем. \n Нажимая ОK ты подтверждаешь передачу своей материальной и интеллектуальной собственности автору программы, а так же даёшь согласие на добровольное чипирование. \n2020 г."
...
#=========================================================================================
root =Tk()
root.title("ALЁ-X-SEY") # Заголовок окна
root["bg"] = "SlateGray1" 
# верхняя менюха
mainmenu = Menu(root) 
...
TMEsearchmenu.add_command(label='Найти на MOUSER', command=TMEsearch_menu_command)
...
visibility_button_frame["bg"] = "SlateGray1"
button_frame=Frame()
button_frame["bg"] = "SlateGray1"
f=LabelFrame()
f["bg"] = "SlateGray1"
f1=Frame(f)
f1_1=Frame(f)
...
Art_visibility_button=Button(visibility_button_frame, text="Арт.[x]", bd=0, bg='SlateGray1', foreground = "white", activebackground = 'yellow', font='Arial 7', height=1, command=visibilityArt)
Art_visibility_button["bg"]="SlateGray1"
#
# ++++++++++++++++++++++++++++++++++++++++КНОПКА СТАРТА++++++++++++++++++++++++++++++++
...
empty_label=Label(f1_1, bg = 'SlateGray1', height=2)
empty_label_b=Label(button_frame, bg = 'SlateGray1',width=10, height=2)
...
label_type =Label(f8,text='Тип по MOUSER(J)',height=2)
...
infolabel=Label(text=TEXT, bg = 'SlateGray1', width=50, height=3)
# привязка событий
...
class visibility:
    def __init__(self,i):
        self.i = i
        self.button = Button(visibility_button_frame, text=visibility_elements[i][0], bd=0, font='Arial 7', height=1, foreground = "white", activebackground = 'yellow', command=self.visibility_on_off)
        if visibility_elements[self.i][6]=='1':
            self.button['text']=visibility_elements[self.i][0]
            self.button['bg']="SlateGray1"
        else:
            self.button['text']=visibility_elements[self.i][1]
            self.button['bg']="SlateGray2"
...
if clmn_key1== '1':
    ...
    Art_visibility_button['text']= "Артикулы"
    Art_visibility_button['bg']= 'SlateGray2'#'SlateGray2'
