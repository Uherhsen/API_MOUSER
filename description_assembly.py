...
from GetDataMouser_with_openpyxl import GetData as Data
...
def checkKey(xlsxpath,jsonfilename):
    ...
    none_quantity = categ_list.count(None)
    while none_quantity > 0:
        categ_list.remove(None)
        none_quantity = categ_list.count(None)
    ...
    for i in range(1, list_range+1):
        ...
        if prod_param != None:
            try:
                ...
                #params_dict.pop('Категория продукта','Нет ключа') # Пока что  не избавляемся
                # УДАЛЕНИЯ ПО КЛЮЧУ
                #
                try:
            ...
            print('Параметров продукта нет на MOUSER')
            continue   
    wb.save(xlsxpath)
